  ...
  return (
    <>
      <Button
        onTap={() => setOpen(true)}
        className={cn(
          "w-fit flex items-center gap-x-2 !bg-[#DFE1DB] px-4 py-2 !rounded-full !text-black ",
          props.className
        )}
      >
      ...
      <Modal onClose={() => setOpen(false)} className="" open={open}>
        <Modal.Body className="p-4 !min-h-0 !h-fit min-w-[300px] shadow-md rounded-xl space-y-4 ">
          <div className="w-full space-y-3 ">
            <Typography
              variant={"p"}
              className="text-black font-semibold w-full pb-2 border-b-2 text-center flex items-center"
            >
              ...
              {months.map((month, index) => {
                return (
                  <Typography
                    ...
                    variant={"span"}
                    className={cn(
                      `font-semibold w-full text-center !mt-0 hover:bg-[#DFE1DB] hover:text-black cursor-pointer `,
                      { "text-black ": dateValue.getMonth() != index },
          ...
          <div className="w-full space-y-3 ">
            <div className="w-full pb-2 border-b-2 flex items-center justify-between">
              ...
              <Typography
                variant={"p"}
                className="text-black !mt-0 font-semibold text-center"
              >
                Select Year
              </Typography>
              <Button
              ...
              {years.map((year, index) => {
                return (
                  <Typography
                    ...
                    className={cn(
                      `font-semibold w-full text-center !mt-0 hover:bg-[#DFE1DB] hover:text-black cursor-pointer `,
                      { "text-black ": dateValue.getFullYear() != year },
                      {
...
const DatePickerComp: React.FC<DatePickerProps> = ({
  ...
  return (
    ...
    >
      ...
      <Typography variant={"h3"} className="w-full mt-4 text-black">
      ...
      <div className={cn(`w-full mt-5 grid grid-cols-7 gap-x-2`)}>
        {daysOfWeek.map((day, index) => (
          <Typography
            key={index}
            variant={"p"}
            className="text-center text-base text-black !mt-0 font-semibold"
          >
            {day}
          </Typography>
        ))}
          ...
          return (
            ...
            >
              <Typography
                variant={"p"}
                className={cn(
                  "text-center text-base text-black !mt-0 font-semibold rounded-full w-10 h-10 flex flex-col items-center justify-center gap-y-0 relative hover:text-black hover:bg-slate-100 ",
                  {
                    "bg-primary-100 ":
